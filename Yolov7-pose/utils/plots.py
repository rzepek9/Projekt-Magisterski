...
KEYPOINTS = ["NOSE", "LEFT_EYE", "RIGH_EYE", "LEFT_EAR", "RIGH_EYE", "LEFT_SHOULDER", "RIGH_SHOULDER", "LEFT_ELBOW", "RIGHT_ELBOW", "LEFT_WRIST", "RIGHT_WRIST", "LEFT_HIPS", "RIGHT_HIPS", "LEFT_KNEE", "RIGHT_KNEE", "LEFT_ANKLE", "RIGHT_ANKLE"]
KEYPOINTS_INTERESTED = ["LEFT_SHOULDER", "RIGH_SHOULDER", "LEFT_ELBOW", "RIGHT_ELBOW", "LEFT_WRIST", "RIGHT_WRIST", "LEFT_HIPS", "RIGHT_HIPS", "LEFT_KNEE", "RIGHT_KNEE", "LEFT_ANKLE", "RIGHT_ANKLE"]
def plot_skeleton_without_head(im, kpts, steps, orig_shape):
    connection = [[0, 2], [1, 3], [2, 4], [3, 5], [6, 8], [7, 9], [8, 10], [9,11]]

    no_kp = len(KEYPOINTS[5:])
    for i in range(no_kp):
...
def calculate_angles(kpts,steps):
    ...
    for i, angle in enumerate(angles):
        pos1 = (int(kpts[angle[0]*steps]) , int(kpts[angle[0]*steps+1]))
        pos2 = (int(kpts[angle[1]*steps]) , int(kpts[angle[1]*steps+1]))
        pos3 = (int(kpts[angle[2]*steps]) , int(kpts[angle[2]*steps+1]))
        ...
        if i == 0:
            left_elbow = calculated_angle
        elif i == 1:
            right_elbow = calculated_angle
        elif i == 2:
            left_knee = calculated_angle
        elif i == 3:
            right_knee = calculated_angle

    return [left_elbow, right_elbow, left_knee, right_knee]

def frame_values(frame, kpts, steps):
    value = [frame]
    angles = calculate_angles(kpts, steps)
    for cord in range(len(KEYPOINTS_INTERESTED)):
        pos = (int(kpts[cord*steps]) , int(kpts[cord*steps+1]))
        value.append(pos)
    for a in angles:
        value.append(a)
