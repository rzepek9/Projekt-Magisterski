...
class LSTM(nn.Module):
    def __init__(self, dropout_v):
        ...
        self.conv_1 = nn.Conv1d(12, 18, 3, stride=1, padding='same')

        self.lstm = nn.LSTM(input_size=51, hidden_size=51, dropout=dropout_v,
                            num_layers=2, batch_first=True)  # lstm

        self.pool = nn.MaxPool1d(3, padding=1)

        self.linear = nn.Linear(14*12, 64)
        self.linear_out = nn.Linear(14*6, 1)
